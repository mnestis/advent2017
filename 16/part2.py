    ...
    for _ in range(1_000_000_000):
        for step in dance_steps:
            if step.startswith("s"):
                ...
                dancers[pos_a], dancers[pos_b] = dancers[pos_b], dancers[pos_a]
        if tuple(dancers) in previous_end_positions:
            repeat_interval = len(previous_end_positions)
            break
        else:
            previous_end_positions.add(tuple(dancers))

    dancers = [chr(i) for i in range(ord("a"), ord("q"))]
    for _ in range(1_000_000_000 % repeat_interval):
        for step in dance_steps:
            if step.startswith("s"):
                offset = int(step[1:])
                dancers = dancers[-offset:] + dancers[:-offset]
